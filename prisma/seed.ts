...
// ─── Message templates ────────────────────────────────────────────────────────
// Templates live in ./message-templates.ts so prisma/sync-templates.ts can
// import and apply them in production without triggering the full seed.
...
async function main() {
  ...
  console.log("→ syncing message templates");
  const tplSync = await syncMessageTemplates(prisma);
  console.log(`  ✓ ${tplSync.total} templates (${tplSync.created} created, ${tplSync.updated} updated)`);
